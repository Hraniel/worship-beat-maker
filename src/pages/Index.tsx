...
import {
  ...
  getCustomSound,
  deleteCustomSound,
  getAllCustomSoundIds,
  saveCustomSoundsForSong,
  loadCustomSoundsForSong,
  deleteCustomSoundsForSong,
} from "@/lib/custom-sound-store";
import {
  addLoop,
  removeLoop,
  setLoopBpm,
  setLoopTimeSignature,
  updateLoopVolume,
  stopAllLoops,
  setMetronomeVolume,
  getMetronomeVolume,
...
const Index = () => {
  ...
  const [padNames, setPadNames] = useState<Record<string, string>>(() => {
    try {
      const d = localStorage.getItem("drum-pads-pad-names");
  ...
  const [padPans, setPadPans] = useState<Record<string, number>>(() => {
    try {
      const d = localStorage.getItem("drum-pads-pad-pans");
      return d ? JSON.parse(d) : {};
    } catch {
      return {};
    }
  ...
  const openMixerGate = useCallback(() => {
    ...
    if (g) {
      ...
      setUpgradeGate({
        gateKey: "mixer_faders",
        gateLabel: "Mixer de faders",
        requiredTier: "pro",
        description: "Controle individual de volume por fader",
      });
    }
  }, [mixerFaderAccess]);
  ...
  const [padColors, setPadColors] = useState<Record<string, PadColor>>(() => {
    try {
      const d = localStorage.getItem("drum-pads-pad-colors");
      return d ? JSON.parse(d) : {};
    } catch {
      return {};
    }
  ...
  useEffect(() => {
    ...
    const handlePopState = () => {
      ...
      if (spotifySheetOpen) {
        setSpotifySheetOpen(false);
        return;
      }
      if (performanceModeOpen) {
        setPerformanceModeOpen(false);
        return;
      }
      if (upgradeGate) {
        setUpgradeGate(null);
  ...
  const handleAudioSettingsChange = useCallback(
    (settings: AudioSettings) => {
      ...
      if (settings.padsStereo === "mono") {
        ...
        Object.keys(padPans).forEach((id) => {
          resetPans[id] = 0;
          setPadPan(id, 0);
        });
  ...
  const handleRenamePad = useCallback((padId: string, name: string) => {
    setPadNames((prev) => {
      const next = { ...prev };
      if (name) next[padId] = name;
      else delete next[padId];
  ...
  const handleApplySpotifyConfig = useCallback(
    (config: any) => {
  ...
  const handleMusicAIAddToSetlist = useCallback(
    async (song: { name: string; artist: string }, bpm: number, key: string | null) => {
  ...
  const songs = setlists.flatMap((sl) =>
    sl.songs.length > 0
      ? sl.songs.map((s) => ({ ...s, id: sl.id, _setlistId: sl.id }))
      : [
          {
            id: sl.id,
            name: sl.name,
    ...
    >
      ...
      {!focusMode ? (
        !currentSongId ? (
          ...
          <header className="relative z-[50] overflow-visible flex items-center justify-between px-3 py-2.5 bg-primary/10 border-b border-primary/20 shrink-0 min-h-[44px]">
          ...
          <header className="relative z-[50] overflow-visible flex items-center justify-between px-2 sm:px-3 py-1 sm:py-2 border-b glass-surface shrink-0">
      ...
      {savedSongsOpen && (() => {
        const closeSavedAndReopen = () => {
          setSavedSongsOpen(false);
          if (reopenSetlistRef.current) {
            reopenSetlistRef.current = false;
            setTimeout(() => setOpenSetlistFromBanner(true), 300);
          }
        };
        return (
        <div className="fixed inset-0 z-50 flex items-center justify-center bg-background/60 backdrop-blur-sm p-4" onClick={closeSavedAndReopen}>
          <div
                  ...
                  <div className="flex items-center gap-1">
                    {!metronomeOpen && (
                      <button
                        ...
                        onClick={(e) => {
                          e.stopPropagation();
                          setMetronomeIsPlaying((prev) => !prev);
                        }}
      ...
      {showSavePrompt && (
        <div className="fixed inset-0 z-50 flex items-center justify-center bg-background/60 backdrop-blur-sm p-4">
          <div
            className="bg-card border border-border rounded-lg shadow-lg p-5 space-y-4 w-full max-w-sm animate-fade-in-up"
