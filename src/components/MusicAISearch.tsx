      ...
      <DialogContent className="max-w-sm">
        ...
        <div className="grid grid-cols-2 gap-3">
          <div className="space-y-1">
            ...
            {song.bpm && editBpm !== String(song.bpm) && editBpm !== '' && (
              <p className="text-[10px] text-muted-foreground">Original: {song.bpm}</p>
            )}
          </div>
          <div className="space-y-1">
            ...
            {song.key && editKey !== song.key && editKey !== '' && (
              <p className="text-[10px] text-muted-foreground">Original: {song.key}</p>
            )}
          </div>
        </div>
        ...
        <DialogFooter className="flex-col gap-2 sm:flex-col">
          ...
          >
            ...
            Solicitar sugestão da IA para os pads
          </Button>
          {onAddToSetlist && (
            <Button
              variant="outline"
...
// ─── Apply Config Popup (edit BPM/Tom before adding to setlist) ───────────────
    ...
    <Dialog open={open} onOpenChange={(v) => { if (!v) onClose(); }}>
      <DialogContent className="max-w-sm">
        ...
        <p className="text-[11px] text-muted-foreground text-center">
          Compasso: <strong>{config.timeSignature}</strong> · Padrão: <strong>{config.patternName || config.recommendedLoop}</strong>
        </p>

        <DialogFooter className="flex-col gap-2 sm:flex-col">
          <Button className="w-full gap-2" onClick={handleConfirm} disabled={!validBpm}>
            <Check className="h-4 w-4" />
            Confirmar e aplicar nos pads
          </Button>
          <Button variant="ghost" className="w-full text-xs" onClick={onClose}>
            Cancelar
          </Button>
...
const MusicAISearch: React.FC<MusicAISearchProps> = ({ onApplyConfig, onAddToSetlist, locked, externalOpen, onExternalOpenChange }) => {
  ...
  // Open info popup when clicking a song result
  const handleSongClick = useCallback((song: AISongResult) => {
    setInfoPopupSong(song);
    setInfoPopupOpen(true);
  }, []);

  // --- AI analysis (called from info popup) ---
  ...
  // --- Apply config (opens confirm popup) ---
  ...
  return (
    <>
      ...
      <SongInfoPopup
        ...
        onAddToSetlist={onAddToSetlist ? handleAddToSetlist : undefined}
      />
                  ...
                  {results.map((r, i) => {
                    ...
                    return (
                      <div key={i} className="relative rounded-lg border border-border/50 hover:bg-muted/60 transition-colors">
                        <button
                          onClick={() => handleSongClick(r)}
                          className="w-full flex items-center gap-3 px-3 py-2.5 text-left pr-16"
                        >
                          {/* Album cover */}
                          <div className="w-12 h-12 rounded-md shrink-0 overflow-hidden bg-muted flex items-center justify-center">
                            {r.albumCover ? (
