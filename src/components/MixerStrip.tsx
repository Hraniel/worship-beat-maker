...
const MixerStrip: React.FC<MixerStripProps> = ({ channels, showAll, compactFaderHeight, controlledPage, onControlledPageChange }) => {
  ...
  if (showAll) {
    return (
      <div className="flex flex-col gap-1">
        <div ref={containerRef} className="flex items-start gap-0.5 px-1 pt-2 pb-1 glass-card rounded-lg overflow-visible">
          {channels.map((ch) => (
  ...
  return (
    <div className="flex flex-col gap-1">
      <div ref={containerRef} className="flex items-start gap-1 px-2 pt-2 pb-1 glass-card rounded-lg overflow-visible">
        {/* Fixed: MET + PAD */}
        {fixedStart.map((ch) => (
          <div key={ch.id} className="flex-1 min-w-0">
            <Fader channel={ch} />
