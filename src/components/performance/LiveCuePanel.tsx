    ...
    <>
      ...
      {isLeader && (
        <>
          ...
          {/* Inline quick buttons — 2-row grid layout */}
          {settings.quickCueButtonsVisible && (
            <div className="relative">
              {songs.length > 0 && (
                <button
                ...
                {CUE_PRESETS.map((cue) => {
                  ...
                  return (
                    <button
                      key={cue.key}
                      onClick={() => sendCue(cue.key)}
                      className={`flex flex-col items-center justify-center gap-0.5 px-2 py-2.5 rounded-xl text-[10px] font-bold text-white ${cue.color} hover:brightness-110 transition-all active:scale-95 ${cue.key === 'worship' ? 'col-span-1' : ''}`}
                      title={cueLabel}
                    >
                      <cue.icon className="h-5 w-5 shrink-0" />
