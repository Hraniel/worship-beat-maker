...
serve(async (req) => {
  ...
  try {
    ...
    // If trackId provided, get basic track info only
    // (audio-features and audio-analysis APIs were deprecated by Spotify in Nov 2024)
    if (trackId) {
      const trackResp = await fetch(`https://api.spotify.com/v1/tracks/${trackId}`, { headers });
      const track = trackResp.ok ? await trackResp.json() : null;

      return new Response(JSON.stringify({ features: null, track, analysis: null }), {
